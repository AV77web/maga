...
export default function ArticoliTable({
  ...
  const fetchRicambi = useCallback(async () => {
    try {
      ...
      const cleanRicambi = res.map((item) => ({
        ...
        has_diba: String(item.has_diba || "").trim().toUpperCase(), // Assicura che il valore sia pulito e maiuscolo
      }));
  ...
  const handleDibaIconClick = async (event, row) => {
    // Se si clicca sulla stessa icona, chiude il popover
    if (popoverTargetId === row.id) {
      handlePopoverClose();
      return;
    }

    setPopoverAnchorEl(event.currentTarget);
    setPopoverTargetId(row.id);
    setPopoverLoading(true);
    setPopoverContent([]); // Pulisce il contenuto precedente

    try {
      const components = await dibaApi.fetchComponentsFor(row.id);
      setPopoverContent(components);
    } catch (error) {
      console.error("Errore nel recupero dei componenti della DiBa:", error);
      setPopoverContent([]); // Gestisce il caso di errore
    } finally {
