...
BEGIN
    ...
    IF p_order_by NOT IN ('id_ordine', 'num_ordine', 'data_ordine', 'fornitore_id', 'stato') THEN
...
BEGIN
    -- ? Dichiarazioni tutte subito dopo BEGIN
    DECLARE v_offset       INT DEFAULT 0;
    ...
    -- ? Restituisci i dati con fallback ORDER BY
    SELECT JSON_ARRAYAGG(
               JSON_OBJECT(
                   ...
                   'totale_righe',    (quantita * prezzo_unitario)
               )
           ) AS data
    FROM ordini_righe
    WHERE (p_id_ordine IS NULL OR id_ordine = p_id_ordine)
    ...
    -- ? Restituisci meta-info per frontend
    SELECT JSON_OBJECT(
        'status',     'success',
...
-- Dump completed on 2025-07-22 18:01:04
