...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteOrdini_righe`(
    IN p_id_riga INT
)
BEGIN
    ...
    BEGIN
        ...
        SELECT JSON_OBJECT(
            'status', 'error',
            'message', 'Errore durante la cancellazione della riga ordine'
        ) AS response;
    END;
...
BEGIN
    ...
    FROM
        `fornitori`
    WHERE
	    	(p_codice IS NULL OR p_ragione_sociale = '' OR `rag_soc` LIKE CONCAT('%', p_ragione_sociale, '%'))
        AND (p_ragione_sociale IS NULL OR p_codice = '' OR `codice` LIKE CONCAT('%', p_codice, '%'))
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `FetchOrdini_righe`(
    IN p_id_riga INT
)
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT JSON_OBJECT(
            'status', 'error',
            'message', 'Errore durante il fetch delle righe ordini'
        ) AS response;
    END;

    IF p_id_riga IS NULL OR p_id_riga = 0 THEN
        ...
        FROM ordini_righe;
    ELSE
        -- Restituisce solo la riga specifica
        SELECT JSON_OBJECT(
            'id_riga', id_riga,
            'id_ordine', id_ordine,
            'descrizione', descrizione,
...
-- Dump completed on 2025-07-12 23:45:23
