...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `FetchOrdineById`(IN p_id_ordine INT)
BEGIN
    -- This procedure finds a single order and returns it within a structured JSON object,
    -- consistent with the FetchOrdini1 pattern.
    WITH FoundOrder AS (
        SELECT
            id_ordine,
            num_ordine,
    ...
    -- Select the final JSON object. If no order is found, this will return a status of 'not_found'.
    SELECT 
        JSON_OBJECT(
            'status', IF(COUNT(*) > 0, 'success', 'not_found'),
            'data', IF(COUNT(*) > 0, JSON_OBJECT(
                'id_ordine',   fo.id_ordine,
                'num_ordine',  fo.num_ordine,
                'data_ordine', fo.data_ordine,
                'fornitore_id',fo.fornitore_id,
                'stato',       fo.stato,
                'note',        fo.note
            ), NULL)
        ) AS result
...
-- Dump completed on 2025-07-21 17:56:31
