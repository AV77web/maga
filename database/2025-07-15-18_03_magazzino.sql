...
BEGIN
  ...
  WHERE (p_rag_soc        IS NULL OR rag_soc     LIKE CONCAT('%',p_rag_soc,'%'))
    AND (p_tipo           IS NULL OR tipo_cliente = p_tipo)
  ...
  SET @qry = CONCAT(
    ...
    ')) AS data ',
    'FROM (',
      'SELECT id, rag_soc, tipo_cliente, cf, partita_iva, citta, telefono, email, contatto ',
      'FROM   controparti WHERE 1=1',
        IF(p_rag_soc        IS NULL, '', CONCAT(' AND rag_soc LIKE "%',p_rag_soc,'%"')),
        IF(p_tipo           IS NULL, '', CONCAT(' AND tipo_cliente="',p_tipo,'"')),
...
-- Dump completed on 2025-07-15 18:03:55
